...
public class Test {
  ...
  public static void main(String[] args) {
    try (FileReader in = new FileReader("build.gradle")) {
      ...
      // 분석기로부터 보고를 받을 리스너(=observer)를 추가.
      CharacterCountListener l1 = new CharacterCountListener();
      analyzer.addCharacterListener(l1);
      
      // 분석기를 실행.
      // => 분석기는 문자를 한 개 읽을 때마다 분석기에 등록된 모든 리스너에게 보고할 것.
      // => 각각의 리스너는 보고를 받으면 자신의 일을 함.
      
      analyzer.execute();
      
      // 분석 작업이 끝났으면 각 리스너에게 결과를 출력하라고 명령.
      l1.displayResult();
