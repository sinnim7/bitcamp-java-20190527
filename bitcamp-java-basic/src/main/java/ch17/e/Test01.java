...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // sort() 메서드가 반드시 구현돼 있을 것.
    // 따라서 display() 메서드에선 파라미터로 넘어오는 Sorter가
    // BubbleSort 인지, QuickSort 인지, MergeSort인지 따질 필요 없이,
    // 그냥 서브 클래스가 구현한 sort()를 호출하면 됨.
  ...
  static void display(Sorter sorter, int[] values) {
    ...
    // 어떻게 호출할 수 있나? 다음 코드는 오류 아닌가?
    // -> JVM이 sort()를 호출할 때 레퍼런스에 있는 객체가 실제 어떤 클래스의 객체인지 판단.
    //  그런 후 해당 클래스의 메서드를 호출하기 때문에 질문한 것과 같은 문젠 발생 안함.
    // 
    sorter.sort(values);
