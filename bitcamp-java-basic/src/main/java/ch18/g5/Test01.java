...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // 기존 규칙(Printer)에 새 규칙(waterMark())를 추가하더라도
    // 기존 작성한 클래스를 그대로 활용할 수 있음.
    // 굳이 g4 패키지에서 한 것처럼 어댑터 패턴을 사용할 필요도 없음.
  ...
  private static void display(
      Printer printer,   // 기존 규격을 그대로 사용하면 됨.
      String text, 
      String watermarkText) {
    
    printer.waterMark(watermarkText); // 기존 규격에 새로 추가한 메서드.
    printer.print(text);
    printer.waterMark(watermarkText); // 기존 규격에 새로 추가한 메서드.
  }
