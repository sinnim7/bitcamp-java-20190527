...
public class ApplicationContext {
    ...
    for (File f : files) {
      if (f.isDirectory()) {
        ...
        try {
          ...
          if (isComponent(clazz) && !Modifier.isAbstract(clazz.getModifiers())) { //커맨드 인터 구현한 것만 등록하고 && 추상클래스가 아닌 일반만.
  ...
  private boolean isComponent(Class<?> clazz) {
    ...
    // -> 애노테이션 정보를 파라미터로 넘기면 그 애노테이션의값을 리턴한다.
    Component comp = clazz.getAnnotation(Component.class);
    // 애노도 인터처럼 객체로 받을 수 있음.
    if (comp == null)
      return false;
  ...
  private void createCommand() {
    for (Class<?> clazz : classes) { //클래스가져옴.
      // 클래스 Component 애노테이션의데이터를 추출.
      // => 꺼내고자 하는 애노테이션의 타입을 정확하게 지정해야 함.
