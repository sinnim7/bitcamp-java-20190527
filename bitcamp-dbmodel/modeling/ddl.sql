...
-- 학력
DROP TABLE IF EXISTS shcool_grade RESTRICT;

-- 학생
CREATE TABLE students (
  student_id   INTEGER      NOT NULL COMMENT '학생번호', -- 학생번호
  student_path VARCHAR(255) NOT NULL COMMENT '사진', -- 사진
...
CREATE TABLE lectures (
  ...
  start_et   DATE         NOT NULL COMMENT '종료일', -- 종료일
...
CREATE TABLE centers (
  ...
  base_addr VARCHAR(255) NULL     COMMENT '기본주소', -- 기본주소
...
CREATE TABLE add_drop (
  add_id     INTEGER  NOT NULL COMMENT '수강신청번호', -- 수강신청번호
  lecture_id INTEGER  NOT NULL COMMENT '강의번호', -- 강의번호
  student_id INTEGER  NOT NULL COMMENT '학생번호', -- 학생번호
...
ALTER TABLE add_drop
  MODIFY COLUMN add_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '수강신청번호';
...
-- 학력
ALTER TABLE shcool_grade
  ADD CONSTRAINT PK_shcool_grade -- 학력 기본키
    PRIMARY KEY (
      sg_id -- 학력번호
    );

-- 학력 유니크 인덱스
CREATE UNIQUE INDEX UIX_shcool_grade
  ON shcool_grade ( -- 학력
    name ASC -- 학력명
  );
...
-- 회원
ALTER TABLE members
  ADD CONSTRAINT FK_shcool_grade_TO_members -- 학력 -> 회원
    FOREIGN KEY (
      last_sg_id -- 최종학력번호
    )
    REFERENCES shcool_grade ( -- 학력
      sg_id -- 학력번호
    );
