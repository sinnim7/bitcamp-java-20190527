// v32_7 : 수업과 게시물 데이터를 다루는 CRUD를 처리.
...
public class ServerApp {
  ...
  public static void main(String[] args) {
    ...
    try (ServerSocket serverSocket = new ServerSocket(8888)) {
      ...
      try (Socket clientSocket = serverSocket.accept();
        ...
        // 다른 메서드가 사용할 수 있도록 입출력 스트림을 스태틱 변수에 저장.
        ServerApp.in = in;
        ServerApp.out = out;
  ...
  private static void updateMember() throws Exception {
     ...
     * 
     * if (m.getNo() == member.getNo()) { // 기존 객체를 클라이언트가 보낸 객체로 교체. memberList.set(i, member);
  ...
  private static void listMember() throws Exception {
    out.writeUTF("ok");
    out.reset(); // 기존에 serialize 했던 객체의 상태를 무시하고 다시 serialize 함.
